...
if gap_epoch_200 > 20:
    print(" SEVERE OVERFITTING: Gap > 20%")
elif gap_epoch_200 > 10:
    print(" MODERATE OVERFITTING: Gap > 10%")
...
if loss_gap_200 > 0.5:
    print("Test loss >> Train loss (strong overfitting)")
else:
...
if decline > 2:
    print("Test accuracy declining (overfitting in late epochs)")
elif decline > 0.5:
...
if len(overfitting_start) > 0:
    print(f"\nOverfitting starts around epoch {overfitting_start[0]+1} (gap > {overfitting_threshold}%)")
else:
    print(f"\nNo severe overfitting detected (gap never exceeded {overfitting_threshold}%)")

# DETAILED EPOCH-BY-EPOCH LOG
print("\nDETAILED EPOCH-BY-EPOCH LOG (Every 10 epochs)")
...
# FULL TABLE (all 200 epochs to file)
print("\nSAVING FULL EPOCH TABLE")

with open('outputs/results/overfitting_analysis.txt', 'w') as f:
    f.write("COMPLETE TRAINING LOG - ALL 200 EPOCHS\n")

    f.write(f"{'Epoch':>5} | {'Train Loss':>11} | {'Train Acc':>9} | " f"{'Test Loss':>10} | {'Test Acc':>8} | {'Gap':>7}\n")
    
    for i in range(200):
        epoch = i + 1
        gap = train_acc[i] - test_acc[i]
        f.write(f"{epoch:5d} | {train_loss[i]:11.6f} | {train_acc[i]:8.2f}% | "f"{test_loss[i]:10.6f} | {test_acc[i]:7.2f}% | {gap:6.2f}%\n")
